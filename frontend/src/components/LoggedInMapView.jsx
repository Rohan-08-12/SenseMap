      ...
      .then((res) => {
        ...
        if (features.length > 0) {
          setNearbyPlaces(features.slice(0, 6).map((f) => {
            ...
            const comfort = p.comfortScore ?? 0;
            return {
              id: p.id, name: p.name, score: comfort,
              tier: comfort >= 3.5 ? 'high' : 'medium', featured: false,
              tags: p.category || 'Place',
              desc: `Noise: ${(p.noiseScore ?? 0).toFixed(1)} · Lighting: ${(p.lightingScore ?? 0).toFixed(1)} · Crowd: ${(p.crowdScore ?? 0).toFixed(1)}`,
              comfort_score: comfort, noise_score: p.noiseScore, lighting_score: p.lightingScore, crowd_score: p.crowdScore,
              latitude: f.geometry?.coordinates?.[1], longitude: f.geometry?.coordinates?.[0], category: p.category,
            };
          }));
        }
      })
          ...
          <aside className="lmv-detail">
              ...
              <div className="lmv-loc-header">
                <div className="lmv-loc-header-row">
                  <div className="lmv-loc-title">
                    <h2>{locName}</h2>
                    <p>{reviewCount > 0 ? `${reviewCount} reviews` : dataSource === 'estimated' ? 'AI estimate' : 'No data yet'}</p>
                  </div>
              ...
              {(() => {
                ...
                if (dataSource === 'estimated') return (
                  <div style={{ padding: '10px 12px', fontSize: 12, borderRadius: 8, marginTop: 8, background: '#ede9fe', color: '#5b21b6', display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    Scores estimated from Yelp, Foursquare &amp; Reddit via Gemini — no community visits yet.
                  </div>
                );
                return (
            ...
            <div className="lmv-detail-card">
              <div>
                ...
                <p className="lmv-section-desc">{dataSource === 'community' ? 'Generated by AI from community reviews.' : 'Generated by AI from external reviews (Yelp, Google, Foursquare).'} Not a substitute for personal judgement.</p>
              </div>
